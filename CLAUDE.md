...
- **Build status:** see [`docs/BUILD_STATUS.md`](docs/BUILD_STATUS.md) for the full phase-by-phase shipping log. Current state in one line: **V1 polish-complete + Phase 8 + Phase 12 + Phase 13 + Phase 9a + Phase 9b + Phase 9c shipped on `main`** — all 5 modules shipped 2026-05-06 + Phase 6 frontend polish closed 2026-05-08 (12 PRs) + Phase 11 org admin console (11a/11b/11c) + Phase 8 Settings (PR #24) + Phase 12 Auth & Onboarding (PR #25) + Phase 13 Site Setup OSHA + RIDDOR (PR #26) + Phase 9a Argus Foundation (PR #27) + Phase 9b Argus Copilot (PR #28). **Phase 9c Argus Investigator shipped 2026-05-10** (PR #29, commit `a041eea`) — new `ai` tab on `/investigations/[id]` between Findings and Timeline (Sparkles + cyan accent), gated on `orgs.argus_enabled` + `argus:use` + `investigation:edit` + non-closed (URL `?tab=ai` falls back to Summary on any miss); paste + voice/text witness inputs (mic via existing `useVoice()`, witnesses held client-side until first Push); **one-shot Sonnet 4.6 call** with `tool_choice: { type: 'tool', name: 'propose_investigation_draft' }` forced for typed structured output `{ timeline[], whys[5], root_cause_summary, findings, insufficient_input }`; SSE wire `progress → draft → usage → done` (no streaming JSON deltas — full draft lands at once); per-card Push commits via the existing `saveInvestigationText` / `saveWhy` actions with confirm-dialog (Replace / Append / Cancel) when target is non-empty (5-Why is replace-only); `argus_suggestions.outcome` flips to `accepted`/`edited`/`rejected` with diff payload, plus `activity_events` rows (`argus.investigator_drafted` actor_kind='argus' on Generate; `argus.investigator_pushed`/`investigator_rejected` actor_kind='human' on Push/Discard); three-layer hallucination defense (server input gate <50 words → 400; system-prompt rule + redactor; `insufficient_input` escape valve); **zero schema migration** — reuses 9a tables + Phase-2 investigation columns; **zero new RBAC keys** — gates on existing `argus:use` + `investigation:edit`; heavy rate-limit bucket (3/min/user). **Phase 9 — IN PROGRESS** (3 of 5 sub-phases shipped): → 9d magic-wands (5×5 risk-matrix cells + finding→incident escalation + CAPA verification method + OSHA reportability confidence pane) → 9e global side panel page-context awareness + 4 Dashboard `<ArgusInsightTile>` cards + tiles on CAPA/Inspections/Reports → Phase 7 (global search backend → consumes the 6l search shell) → Phase 10 (Safety Bulletin step + wizard 3→4 step restructure). Every runtime change goes through a feature branch + PR per `.claude/rules/github-workflow.md`; direct push to `main` is reserved for doc-only updates the user explicitly asks for.
