...
- **Build status:** see [`docs/BUILD_STATUS.md`](docs/BUILD_STATUS.md) for the full phase-by-phase shipping log (Phases 0 → 11c all shipped; 6d–6j queued). Current state in one line: **V1 demo feature-complete on `main` (all 5 modules shipped 2026-05-06)**; Phase 6 frontend polish in progress (6a/6b/6c/6k/6l shipped, 6d–6j queued); **Phase 11 org admin console complete (11a sites + 11b members + 11c roles + invitations all shipped 2026-05-07)** — every org-admin task that previously required the Supabase SQL editor now has a UI path. Every runtime change goes through a feature branch + PR per `.claude/rules/github-workflow.md`; direct push to `main` is reserved for doc-only updates the user explicitly asks for.
