...
- **Build status:** see [`docs/BUILD_STATUS.md`](docs/BUILD_STATUS.md) for the full phase-by-phase shipping log. Current state in one line: **V1 polish-complete + Phase 8 + Phase 12 + Phase 13 + Phase 9a + Phase 9b shipped on `main`** — all 5 modules shipped 2026-05-06 + Phase 6 frontend polish closed 2026-05-08 (12 PRs: 6a Dashboard #7 · 6k Sidebar #8 · 6l Topbar #9 · 6b Incidents #10 · 6c Investigations #11 · 6d CAPA #17 · 6e Reports #18 · 6f Templates #19 · 6g Inspections #20 · 6h Resources #21 · 6i Planner #22 · 6j Admin #23) + Phase 11 org admin console (11a sites #14 · 11b members #15 · 11c roles + invitations #16) + Phase 8 Settings shipped 2026-05-09 (PR #24 — `/settings` account preferences) + Phase 12 Auth & Onboarding merged 2026-05-09 (PR #25 — public `/register` · `/verify-otp` (DEMO OTP `8484`) · `/forgot-password` · `/reset-password` · 3-step `/onboarding` wizard backed by `bootstrap_org_v1` RPC; auth gate split; proxy middleware uses `getUser()` for auto-refresh) + Phase 13 Site Setup OSHA + RIDDOR alignment merged 2026-05-09 (PR #26 — wizard reshape from 7 numeric steps to 9 slug-based steps `basics → jurisdiction → identifiers → workforce → hazards → departments → people → recipients → confirm` with country branching at every layer; 22 new columns on `sites` covering structured address + lat/long + jurisdiction + EIN/NAICS/CRN/UK SIC identifiers + workforce + applicable_standards/hazard_tags `text[]` + EHS lead + RIDDOR responsible person; new `site_emergency_contacts` child table; computed-read SQL functions `is_ita_required` + `is_partially_exempt` from 29 CFR 1904 Appendix A; INSERT/UPDATE/DELETE RLS policies on `notification_recipients` (closed a Phase-0-era SELECT-only bug that blocked Step 6 saves); localStorage draft persistence on every step's form survives tab-close / refresh / browser crash with a "Draft restored" banner + Discard CTA, 24h TTL, auto-clears on next-step mount + on Launch). Surfaces audited across all 35 routes; every org-admin task that previously required the Supabase SQL editor now has a UI path; brand-new emails self-onboard end-to-end with no service-role intervention. **Roadmap:** Phase 9a Argus Foundation merged 2026-05-10 (PR #27); Phase 9b Argus Copilot merged 2026-05-10 (PR #28, commit `6d1c54c`) — inline `<ArgusFormAssistant>` collapsible strip mounted on **all 3 Report Wizard steps** above WizardProgress (matches `assets/incident-reporting.png` and `incident-reporting-2.png`); mic via Web Speech API (`useVoice()`); camera capture via `<input capture="environment">`; **four** Anthropic tool-use schemas (`update_incident_field` for auto-fill of `type`/title/description/area/location/substance/equipment + `log_observation` + `attach_photo` + `raise_stop_work`); agentic loop in `app/api/argus/copilot/route.ts` with prompt caching on system + tools; new `ensureBlankDraft()` server helper pre-creates the draft on Step 1 first load so the strip has an `incidentId` to target from the start (legacy `createDraft` kept for the helper's INSERT path; new `saveStep1` UPDATEs from form submit); Step components carry `key={incident.updated_at}` so they remount with fresh server values after `router.refresh()`; wizard padding reset to `space-y-4` matching `/incidents` and other list pages; new `notification_kind='stop_work_raised'` enum value; sticky `<StopWorkBanner>` on (app) shell with one-click Acknowledge; `profiles.argus_copilot_disabled` per-user opt-out. **No floating FAB** — the global topbar Sparkles (9a) is the org-wide Argus chat surface; the wizard strip is the contextual one. **Phase 9 — IN PROGRESS** (2 of 5 sub-phases shipped): → 9c AI Investigator on `/investigations/[id]` (paste description + voice/text witnesses → streamed timeline + RCA narrative draft via Sonnet 4.6 → "Push to investigation" approval gate) → 9d magic-wands (5×5 risk-matrix cells + finding→incident escalation + CAPA verification method + OSHA reportability confidence pane) → 9e global side panel page-context awareness + 4 Dashboard `<ArgusInsightTile>` cards + tiles on CAPA/Inspections/Reports → Phase 7 (global search backend → consumes the 6l search shell) → Phase 10 (Safety Bulletin step + wizard 3→4 step restructure). Every runtime change goes through a feature branch + PR per `.claude/rules/github-workflow.md`; direct push to `main` is reserved for doc-only updates the user explicitly asks for.
