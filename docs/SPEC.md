...
| 2026-05-12 | **Phase 17 — Settings redesign + self-service account/org deletion** | Migration `20260524120000_phase17_settings_redesign.sql` lands `orgs.logo_url`, `profiles.{sidebar_hidden_items, argus_panel_default}`, `user_notification_silences`, the new `org:configure` permission key (granted to `site_admin` via the existing catch-all in `seed_default_roles()`), `count_org_configure_holders` + `org_delete_summary` SECURITY DEFINER RPCs, and a public `org-logos` Storage bucket with per-org path-prefix RLS. Closes the Phase 8 SPEC §15 deferral on per-user notification silencing. `/settings` shifts from a 4-card single-column page to a left-sidebar IA with 4 groups / 9 nested-route tabs (Profile · Appearance · Sidebar · Organization · Argus · Notifications · Security · Cookies · Delete account) — see new §UI-SETTINGS-IA. Self-service account deletion ships per new §AUTH-DELETE-ACCOUNT with a **two-unlock last-admin gate**: promote another admin OR delete the entire organisation along with the account. The org-delete escape hatch uses a stricter "type the org name + the literal word DELETE" gate plus a count summary (members / sites / incidents). Migration also flips 7 FKs from `RESTRICT` to `SET NULL` on `profiles(id)` so hard-delete can complete without constraint violations (`capas.owner_id`, `severity_overrides.overridden_by`, `hazard_risk_assessments.assessor_id`, `incident_hazard_links.identified_by`, `jsas.created_by`, `jsa_signoffs.worker_id`, `jsa_incident_links.identified_by`); the "every CAPA has an owner" invariant moves from the FK to the create-CAPA action. Non-goals (hard cuts): no Branding tab, no Billing tab, no Invoices tab, no Integrations move from `/admin/integrations`, no Privacy & Data tab, no org-admin Argus on/off toggle, no token-budget edit, no editable workspace slug, no avatar upload, no MFA, no active-sessions list, no per-user timezone, no language preference, no audit-log surface on org edits, no GDPR "Export my data" endpoint, no real cookie consent toggles (app uses essential cookies only). |
...
## §UI-SETTINGS-IA Settings IA (Phase 17)

Locks the structure of the `/settings` surface so future tabs land in the same place and the boundary against `/admin` stays clear.

**Route shape:** `/settings/<section>`. Bare `/settings` redirects to `/settings/profile`. Sections are nested routes, not query params or tab state — each tab is bookmarkable, the back button works, and Next.js streams each tab independently.

**4 groups / 9 tabs:**

| Group | Tab | Route | Visibility |
|---|---|---|---|
| **Account** | Profile | `/settings/profile` | Authenticated user |
| **Account** | Appearance | `/settings/appearance` | Authenticated user |
| **Account** | Sidebar | `/settings/sidebar` | Authenticated user |
| **Workspace** | Organization | `/settings/organization` | `org:configure` |
| **Preferences** | Argus | `/settings/argus` | Authenticated user |
| **Preferences** | Notifications | `/settings/notifications` | Authenticated user |
| **Security** | Security | `/settings/security` | Authenticated user |
| **Security** | Cookies | `/settings/cookies` | Authenticated user |
| **Security** | Delete account | `/settings/delete-account` | Authenticated user |

**Permission gate:** `Organization` is the only permission-gated tab. The sidebar hides it for non-admins; the page itself returns the `EmptyState` pattern if a non-admin types the URL directly. Org-scope check uses `orgCan("org:configure")` (Phase 3 helper); the perm key is granted to `site_admin` only.

**`/settings` vs `/admin` boundary** — `/settings` is **configuration**: your account, your workspace's identity (name / industry / logo), your preferences. `/admin` is **operations**: sites, members, roles, invitations, integrations, site-setup wizards, demo reset. The Integrations card (Phase 16 SDS stub) stays under `/admin/integrations`. This boundary is load-bearing: future surfaces that fit the verb "configure" land in `/settings`; verbs like "manage" or "run" land in `/admin`.

**Sidebar customisation (per-user)** — `profiles.sidebar_hidden_items text[]` stores the nav hrefs the user has hidden. `AppSidebar` filters NAV_ITEMS against the array after the RBAC gate. `PINNED_NAV_HREFS = {/dashboard, /incidents/new/1, /admin}` are unhideable even if a stale value sits in the array (defence in depth). Hidden items remain reachable via direct URL and ⌘K search — the RBAC gate is the only access boundary, the hide list is purely visual.
...
Satisfies GDPR Article 17 ("right to be forgotten") with a typed-email confirmation flow.

**Hard delete via `auth.admin.deleteUser(user.id)`** — cascades through `auth.users → profiles → site_members / team_members`. Authored rows on operational tables (`incidents.reporter_id`, `investigations.lead_investigator_id`, `capas.owner_id`, `safety_bulletins.created_by`, `hazards.identified_by_user_id`, `jsas.created_by`, etc.) have `ON DELETE SET NULL` — author becomes **"Removed user"** in the UI.

**Migration `20260524120000_phase17_settings_redesign.sql` flips 7 FKs from `RESTRICT` to `SET NULL`:**
- `capas.owner_id`
- `severity_overrides.overridden_by`
- `hazard_risk_assessments.assessor_id`
- `incident_hazard_links.identified_by`
- `jsas.created_by`
- `jsa_signoffs.worker_id`
- `jsa_incident_links.identified_by`

The "every CAPA has an owner" invariant moves from the FK to the create-CAPA action. Existing data still satisfies the invariant; orphan ownership only happens after a deletion, and an admin can reassign via the existing flow.

**Last-admin guard — two unlocks:** the deleting user runs a server-side count via `count_org_configure_holders(p_org_id)` (SECURITY DEFINER SQL fn). If the result is `<= 1` (the deleter is the sole admin), the action returns an error and the UI shows two paths:

1. **Promote another admin first** — link to `/admin/members`, then come back and delete the account (org stays).
2. **Delete the entire organisation** — a separate `deleteOwnOrg` server action with a stricter two-field gate (type the org name AND the literal word `DELETE`). Cascades: service-role enumerates every profile in the org, deletes the `orgs` row (which cascades through every `org_id` FK including `profiles.org_id` itself), then cleans up the orphan `auth.users` rows via `admin.auth.admin.deleteUser`. Redirects to `/login?org_deleted=1`.

**Confirmation gates** (both flows):

| Flow | Gate fields |
|---|---|
| Delete account | Type your own email (must match `user.email`, case-insensitive) |
| Delete entire org | Type the exact org name **AND** type the literal word `DELETE` |

**Audit:** an `account.deleted` `activity_events` row is inserted **before** the cascade fires, with the email captured in payload. For org delete, an `org.deleted` row goes in before the cascade. Both are append-only audit rows (`activity_events.UPDATE/DELETE` are revoked from PUBLIC per Phase 0).

**Login banners:**
- `/login?account_deleted=1` → success banner "Your account has been deleted. Thank you for using the platform."
- `/login?org_deleted=1` → success banner "The organisation has been deleted. All accounts and data within it were permanently removed."

**Non-goals:**
- No soft-delete fallback — `profiles.deleted_at` would add a column + a hide-by-default filter to every list query. GDPR right-to-be-forgotten is not satisfied by soft delete.
- No "Export my data" GDPR endpoint in v1 — that's a v2 feature requiring a backgrounded job + signed-URL deliverable.
- No grace period (no 14-day "we can restore this" window). Permanent and immediate, mirroring the typed-confirmation gate.
