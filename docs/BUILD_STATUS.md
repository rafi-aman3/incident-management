...
For hard rules, tech stack, and the doc-reading order, see `CLAUDE.md`. For tenancy, RBAC, data model, and workflow rules, see `docs/SPEC.md`.
...
Shipped 2026-05-11 on `feat/phase-7-global-search` (PR #32, commit `f464d7e`). Demo-grade ⌘K command palette hydrating the Phase 6l search shell. One route `app/api/search/route.ts` running `Promise.all` ILIKE fan-out across all 9 module entities (Incidents · Investigations · CAPAs · Inspections · Templates · Assets · Documents · Sites · People), `LIMIT 5` per group, RLS gates visibility per user, soft-delete (`deleted_at` / `archived_at`) filtered per table, empty groups dropped from the response. New `components/ui/command.tsx` shadcn Command primitive on top of `cmdk` 1.1.1 (one new dep, ~20 kB). New `lib/search/escape.ts` escapes `% _ , \ )` so user input can't break PostgREST `.or()` filter syntax. `components/app-shell/search-trigger.tsx` body rewrite: 8 "Jump to" module shortcuts as the empty state (Dashboard + 7 module list pages); "Recent searches" group backed by `localStorage` key `argus.search.recent` (MRU 5, written only on successful navigation from a result); 200 ms debounced fetch with `AbortController`; `cmdk shouldFilter={false}` (server is the filter); global ⌘K / Ctrl+K binding with a guard so it doesn't hijack input/textarea/contentEditable elsewhere (still works from inside the palette's own input). No FTS / `tsvector` / GIN / `pg_trgm` — ILIKE is plenty for demo data; future migration to `.textSearch()` is a route-internal swap, API shape stays stable. Zero schema migration, zero new RBAC keys, no telemetry.

---

## Phase 10 — Safety Bulletins

Shipped 2026-05-11 on `feat/phase-10-safety-bulletins` (PR #33, commit `f8b503b`). Standalone post-investigation lessons-learned module per IMS_PLANNING §6.4.3 + §12.2.16. Demo-grade scope (author + list + view + dashboard widget); audience targeting + acknowledgement tracking + worker "unread" banner all deferred to Phase 10.5. **The "Report Wizard 3→4 step restructure" framing in the original Phase 10 description was dropped** — bulletins describe lessons learned which workers don't know at report time; authoring belongs after investigation closure. SPEC §15 gets a divergence note. Shipped: new `safety_bulletins` table + `safety_bulletin_status` enum + RLS (read = published OR archived OR author; insert = `bulletin:create`; update = author-of-draft OR `bulletin:publish`; no delete) + two permission keys (`bulletin:create` / `bulletin:publish` → `ehs_manager` + `site_admin`) + `seed_default_roles()` update. Routes: `/bulletins` (Published / My drafts / Archived filter chips), `/bulletins/new` (accepts `?from_investigation={id}` and pre-fills title + HTML body via `lib/bulletins/seed-from-investigation.ts`), `/bulletins/[id]` (sanitized HTML render via `isomorphic-dompurify`, Edit/Publish/Unpublish/Archive buttons gated on RBAC + ownership), `/bulletins/[id]/edit` (drafts only, author only). Server actions: `createBulletin` / `updateBulletin` / `publishBulletin` / `unpublishBulletin` / `archiveBulletin`. **Body is rich-text (TipTap, `@tiptap/react` + `@tiptap/starter-kit` + `@tiptap/extension-link`), NOT markdown** — EHS authors are not markdown-fluent; the editor exposes Bold/Italic/H2/H3/bullet/numbered list/blockquote/inline code/link/undo/redo and stores HTML in the existing `body text` column (no schema change). Mount points: sidebar entry between **Reports** and **Admin** (read implicit, no permission gate); dashboard widget `<LatestBulletinsCard>` (top-3 published org-wide, renders nothing if empty); investigation Summary tab `<BulletinCta>` gated on `closed && incident.track === 'A' && bulletin:create && no non-archived bulletin from this investigation` (swaps to "Bulletin drafted: …" link when one exists). New deps: `@tiptap/react` / `@tiptap/starter-kit` / `@tiptap/extension-link` / `isomorphic-dompurify` (replaced the short-lived `react-markdown` from the initial commit). **All v1-roadmap phases now shipped.**

---

## Topbar responsive fix

Shipped 2026-05-11 on `fix/topbar-responsive` (PR #34, commit `994b2ee`). At 375px the topbar overflowed (7 controls in one row). Moved the SiteSwitcher into the mobile sidebar Sheet below `lg`; topbar at narrow widths is now just the icon cluster. Sheet variant uses an **inline collapsible list** (no Radix popover portal — the dropdown was breaking out of the Sheet edges). Same component (`components/app-shell/site-switcher.tsx`) with two internal variants: `TopbarSiteSwitcher` keeps the Radix dropdown for ≥ lg; `SheetSiteSwitcher` is a pure inline `<button>` + `<ul>` with `max-h-[40vh] overflow-y-auto`. Threaded site props through `SidebarShell` + `AppSidebar`; nav-item clicks now also `setOpenMobile(false)` so the Sheet auto-closes after navigation. Zero schema, zero new deps. **Durable rule:** inside mobile Sheets / Drawers, prefer inline collapsibles over Radix Popover-portaled UI — popovers portal to body and can overflow the drawer edge regardless of width caps.

---

## SPEC §HZ Hazard Register — spec adoption

Adopted 2026-05-11 on `docs/spec-hz-hazard-register` (PR #35, commit `1f33e2a`). New SPEC section locking the v2 Hazard Register module per ISO 45001 §6.1.2. **Spec only, no code.** 5 new tables (`hazards` 6-state lifecycle / `hazard_risk_assessments` history-preserving with 8 trigger types / `hazard_controls` 5-level hierarchy elimination→PPE / `hazard_candidates` review queue from 6 identification methods / `incident_hazard_links` causal·contributing·exposed_but_not_causal feedback loop), routes `/hazards/*` + `/hazards/candidates/*`, 8 components, 3 action files, 4 KPI tiles (identified · S4-S5 high-risk · overdue reviews · **PPE-only-control auditor warning**), 5 new permission keys. **Shared 5×5 risk matrix to extract to `lib/risk/matrix.ts`** so the existing incident severity engine + new hazard risk assessment consume one lookup; `computeResidual()` applies max-reduction by highest applied control level. SDS Manager integration stubbed in v1 (text column present, no live API). Implementation queued for the v2 surface scoping pass.

---

## SPEC §JSA Job Safety Analysis — spec adoption

Adopted 2026-05-11 on `docs/spec-jsa` (PR #36, commit `3517314`). New SPEC section locking v2 §JSA per OSHA 3071 / HSE INDG163. **Spec only, no code.** 5 new tables: `jsas` (5-state lifecycle `draft → under_review → approved → expired → archived`, `expires_at` default = `approved_at + 12 months`, `approved_by != created_by` enforced at DB CHECK), `jsa_steps` (`@dnd-kit`-reorderable, `UNIQUE (jsa_id, sequence)`), `jsa_step_hazards` (FKs into `hazard_candidates(id)` + `hazards(id)` — depends on §HZ shipping first), `jsa_step_controls` (same 5-level hierarchy as §HZ), `jsa_signoffs` (`UNIQUE (jsa_id, worker_id, signed_for_session)` so workers re-sign per shift). 4-step wizard (Identity → Steps → Hazards & Controls → Review & Approve). Selective step-hazard promotion to §HZ via `hazard_candidates` rows with `source_type='jsa'`. **Approver ≠ creator** invariant uses the project's three-layer pattern (DB CHECK + server action + UI). 5 proposed permission keys (`jsa:read_site / draft / approve / signoff / promote_step_hazard`). Shares the §HZ 5×5 risk matrix via `lib/risk/matrix.ts`. **Build order: §HZ → §JSA.**

---

## SPEC §SDS-INTEGRATION SDS Manager stub — spec adoption

Adopted 2026-05-11 on `docs/spec-sds-manager-integration` (PR #37, commit `4e56922`). Third v2 module spec'd in `docs/SPEC.md` §SDS-INTEGRATION. Demo-grade stub for when the real SDS Manager (separate sub-company app) ships its API. **No new tables** — writes into `hazard_candidates` from §HZ via batch import. Surface: `/admin/integrations` (not `/settings` — integrations are org-scoped, account preferences live elsewhere) shows an "SDS Manager" card with a configurable external link (`NEXT_PUBLIC_SDS_MANAGER_URL`). `/hazards/candidates` gets an "Import from SDS Manager" button → opens `<SdsImportModal>` listing 6 hardcoded chemicals from `lib/sds/dummy-catalog.ts` (Toluene · Sulfuric Acid · Hydraulic Oil · Isopropanol · Acetone · Sodium Hydroxide; realistic GHS pictograms + H-statements + suggested controls). Multi-select → batch creates one `hazard_candidates` row per H-statement with `source_type='sds_import'` + `proposed_metadata` jsonb carrying SDS reference + suggested controls. When real API credentials arrive: swap `DUMMY_SDS_CATALOG` for an API call in `actions/sds.ts` — candidate creation flow stays identical. Explicit non-goals: no `sds_records` table, no `sds_chemical_uses`, no SDS revisions in v1 stub. **Depends on §HZ shipping first.**

---

## V2 phases 14-16 research + implementation plans

Landed 2026-05-11 on `docs/v2-phases-14-16-plans` (PR #38, commit `e2019a3`). Competitor research (Cority / Intelex / VelocityEHS / EcoOnline) + standards cross-check (ISO 45001 §6.1.2 / OSHA 3071 / HSE INDG163 / OSHA HazCom 2012+2024 / GHS Rev 9) synthesized at `docs/superpowers/specs/2026-05-11-v2-phases-14-16-research.md` (451 lines). Three implementation plans drafted: `plans/14-hazard-register.md` (§HZ, 593 lines), `plans/15-jsa.md` (§JSA, 461 lines), `plans/16-sds-manager-stub.md` (§SDS-INTEGRATION, 361 lines). Locked specs research-validated; **8 spec deltas folded into plans** (3 §HZ + 4 §JSA + 1 §SDS) to land alongside the migrations: (a) §HZ add `hazard_controls.next_control_review_at date` + KPI tile feeds both cadences; (b) §HZ risk-owner auto-notification on `incident_hazard_links` insert; (c) §HZ new Argus `suggest_hazard_controls` wand on `convertCandidate` form (Phase 9d pattern); (d) §JSA 12-month expiry is **policy** not standards (neither OSHA 3071 nor HSE INDG163 mandates a fixed period — both require event-triggered review); (e) §JSA new `jsa_incident_links` table + causal-link auto-flip mirroring `incident_hazard_links`; (f) §JSA explicit non-goal: permit-to-work execution is v2+ scope (all 4 competitors sell a separate PTW module); (g) §JSA mobile photo evidence per step via existing `documents` polymorphic table (`linked_to='jsa_step'` + `'jsa_step_hazard'`); (h) §SDS GHS pictograms passed through into `proposed_metadata.pictograms`. **Sequencing locked:** §HZ → §JSA → §SDS confirmed mechanical (FK chain); §SDS stub plan recommends shipping in **same PR as §HZ** (~300-500 LOC, zero migrations, zero new tables — writes into Phase 14 tables only) to avoid a bare-candidate-queue interim state. **Refactor commitment:** `lib/risk/matrix.ts` + `lib/risk/types.ts` + `lib/risk/coords.ts` shipped in commit 1 of Phase 14 — existing `lib/workflow/severity.ts` becomes a thin adapter (`MatrixCoord` integer API preserved for incidents wizard via `lib/risk/coords.ts`; new string-named API for hazards + JSA). Backlog for v2.1/v3: bowtie viz, FLRA/FLHA layer, parallel residual-risk modelling, `suggest_step_hazards`/`suggest_step_controls` Argus wands (stricter guardrails), personnel competency gating, configurable risk methodology, multilingual JSA body, catalog swap for broader pictogram coverage. Plans match existing `plans/NN-*.md` project convention (status / goal / branch / spec deltas / schema / actions / components / RBAC / Argus / verification / open Q&A).

---

## Phase 14 Hazard Register + Phase 16 SDS Manager stub

Shipped 2026-05-11 on `feat/phase-14-hazard-register` (PR #40, commit `a822958`). **Combined v2 PR** per the plan recommendation. Shared 5×5 matrix extracted to `lib/risk/{types,matrix,coords}.ts`; `lib/workflow/severity.ts` is now a thin integer-coord adapter (no behavior change for incidents — all 25 cells return identical severity codes). Migration `20260521120000_phase14_hazard_register.sql` ships 5 new tables (`hazards` 6-state lifecycle with `HAZ-<SITE>-YYYY-NNNN` ref-code via `generate_hazard_ref_code()` / `hazard_risk_assessments` history-preserving with 8 trigger types / `hazard_controls` 5-level hierarchy with `next_control_review_at` cadence delta / `hazard_candidates` queue across 8 inbound source types / `incident_hazard_links` feedback loop with 3 link types). 5 new permission keys (`hazard:read_site` / `hazard:report` / `hazard:manage` / `hazard:close` / `hazard_candidate:review`) with backfill grants on the 4 default roles + `seed_default_roles()` patched. New `notification_kind='hazard_incident_linked'` enum value; risk-owner auto-notification fires on every `incident_hazard_links` insert (notifies `hazard.identified_by` + current RA `assessor_id`); on a `link_type='causal'` link against a controlled/monitoring hazard, status flips to `under_assessment` (auditor-defensible — prior assessment invalidated).

**SPEC §HZ.3 matrix orientation corrected at kickoff Q&A** — the drafted §HZ matrix inverted S1↔S5 (rare+insignificant → S1, almost_certain+catastrophic → S5) which would have flipped incident severity for every cell; build phase rewrote §HZ.3 to match the shipped scale (S1=Critical/worst … S5=Insignificant/best), updated §HZ.8 high-risk KPI criterion to `('S1','S2')`, and `computeResidual()` now caps at S5 (reductions move S-number UP toward safer).

UI surface: 5 routes under `app/(app)/hazards/*` (list with 4 KPI tiles incl. amber-styled PPE-only-controls auditor warning · new · detail with 4 tabs Overview/Assessments/Controls/Linked-Incidents · candidates queue with source-type filter chips · candidate review with Convert/Dismiss/Merge modes). 10 components in `components/hazards/*` + `components/risk-matrix/risk-matrix-hazard.tsx` (string-named picker alongside the integer one). Sidebar gets one new top-level entry between Inspections and Resources (`TriangleAlert` icon, gated on `hazard:read_site`). Investigation findings tab grows an `<IncidentHazardLinkSection>`. Argus: 6th wand surface `suggest_hazard_controls` with new tool definition + system prompt encoding the ISO 45001 hierarchy explicitly + `ppe_only_warning` flag surfaced as amber alert; standalone `<ArgusHazardControlsWand>` mounts on the candidate-review form Convert tab. Page-context provider extended with 4 new `ArgusRouteKey` values.

**Phase 16 SDS Manager stub** ships in the same PR: `lib/sds/dummy-catalog.ts` with 6 hardcoded chemicals (Toluene · Sulfuric Acid 98% · Hydraulic Oil ISO VG 46 · Isopropanol 99% · Acetone · Sodium Hydroxide 50%) each with realistic CAS / GHS pictograms / signal words / H-statements / hierarchy-ordered suggested controls. `<GhsPictogram>` component renders the standard red-bordered diamond with a lucide icon stand-in per code. `importSdsHazards()` server action in `lib/actions/sds.ts` writes into `hazard_candidates` — one row per H-statement, with `proposed_metadata` carrying `sds_id` / `product_name` / `cas_number` / `pictograms[]` / `signal_word` / `suggested_controls[]` so the §HZ `convertCandidate` form pre-fills controls. New route `/admin/integrations` + `<SdsIntegrationCard>` with external link to `NEXT_PUBLIC_SDS_MANAGER_URL` and a deep-link to `/hazards/candidates?action=import-sds` that auto-opens the modal. Zero new tables / zero new RBAC keys for Phase 16 — `hazard:report` gates the import.

**Hotfix `20260521130000_fix_hazard_ref_code_site_column.sql`** corrects `generate_hazard_ref_code()` which referenced a non-existent `sites.short_code` column; the function now derives the 3-letter site prefix from `sites.name` (alphanumeric → upper → first 3 chars). Insight tile on `/hazards` deferred to a polish PR (the 4 KPI tiles already cover the same signals). New deps: none. New env: `NEXT_PUBLIC_SDS_MANAGER_URL`.

---

## Phase 15 — JSA (Job Safety Analysis) + 2 Argus wands

Shipped 2026-05-12 on `feat/phase-15-jsa` (PR #41, commit `d716135`). Migration `20260522120000_phase15_jsa.sql` ships 5 §JSA-spec tables (`jsas` 5-state lifecycle / `jsa_steps` @dnd-kit reorder / `jsa_step_hazards` 5×5-matrix-scored / `jsa_step_controls` 5-level hierarchy / `jsa_signoffs` unique on `(jsa_id, worker_id, coalesce(signed_for_session, ''))`) + new `jsa_incident_links` table (mirrors §HZ's `incident_hazard_links` — causal link auto-flips `approved → under_review`) + 5 new permission keys (`jsa:read_site / draft / approve / signoff / promote_step_hazard`) + `seed_default_roles()` patched + `notification_kind='jsa_review_required'` enum value + `document_link_parent` extended with `'jsa_step'` and `'jsa_step_hazard'` for mobile photo evidence via the existing polymorphic `documents` table + `expire_overdue_jsas()` SQL function + defensive `pg_cron` schedule at 01:15 UTC daily (skipped if pg_cron not enabled, idempotent re-runs).

**Three SPEC deltas folded into the migration:** (1) `jsa_incident_links` table + causal auto-flip; (2) 12-month `expires_at` is **policy** not standards (OSHA 3071 + HSE INDG163 both leave the period undefined); (3) `permits_required` is descriptive metadata only — explicit non-goal as a permit-issuance workflow.

5 routes under `app/(app)/jsa/*` (list with status tabs · new Step-1 form · detail with all sections inline · edit `?step=steps|hazards|approve` 3-step wizard · perform worker sign-off page). 8 components under `components/jsa/*` (`jsa-list` · `jsa-status-badge` · `jsa-step1-form` · `jsa-wizard` with internal DndContext + Sortable steps + per-hazard RiskMatrixHazard + per-hazard controls list · `jsa-detail-view` · `jsa-promote-button` · `jsa-signoff-form` · `incident-jsa-link-section` mounted on the investigation Findings tab alongside the existing hazard-link section · `argus-step-hazards-wand` + `argus-step-controls-wand`).

**Two Argus wands pulled forward from v2.1 into this PR** per kickoff Q&A: `suggest_step_hazards` (per step on Step 3) and `suggest_step_controls` (per hazard on Step 3) — both gated on `jsa:draft`, smart-tier with `thinking: 'auto'`, with an **explicit "I've reviewed these" checkbox required before Accept enables** (stricter than CAPA-metadata one-click Accept because workers act on the JSA contents; incorrect PPE recommendations are life-critical). PPE-only outputs flash an amber ISO 45001 §8.1.2 auditor warning. Three-layer "approver ≠ creator" enforcement: DB CHECK constraint + action-layer guard + Approve view in wizard shows different copy for creator (cannot approve, must wait for someone else) vs eligible approver (sees the expiry-date picker + Approve button). `updateJsa` does a full transactional wipe + re-insert of the step tree per SPEC §JSA.7 (cascade FK makes it cheap — ~100 rows on a 10×3×3 JSA).

Sidebar gets a new top-level entry **JSA** between **Hazards** and **Resources/Assets** using lucide `HardHat`. Page-context provider extended with 4 new route keys (`jsa_index`, `jsa_detail`, `jsa_edit`, `jsa_perform`) and per-route panel suggestion chips. `hasActiveSignal` on the index fires when `under_review > 0 || expired_unaddressed > 0`. The `unpublishJsa` path lets an author with `jsa:draft` (or any holder of `jsa:approve`) revert an approved/under-review JSA back to draft for editing; `archiveJsa` is the soft retirement. No new top-level deps.

---

## Phase 17 — Settings Redesign (plan only)

Plan merged 2026-05-12 on `docs/phase-17-settings-redesign-plan` (PR #42, commit `1874ff4`). `plans/17-settings-redesign.md` informed by Q&A in-session + reference image at `assets/settings.png`. **Plan only, no code yet — implementation kicks off after the 8 open-Qs Q&A round.** Re-architects `/settings` from the Phase-8 single-column 4-card account-preferences surface into a left-sidebar IA with **9 nested-route tabs across 4 groups** (Account · Workspace · Preferences · Security). Adds first-class **Organization edit** (name / industry / logo) for site-admins, **per-user sidebar customisation** (hide module rows you don't use — e.g. Assets / Documents), **per-user-per-kind notification silencing** (closes the Phase 8 SPEC §15 deferral; life-safety kinds `stop_work_raised` + regulatory clock kinds are non-silenceable at both bell-feed AND dashboard-banner read layers), **per-user Argus side-panel default** (`profiles.argus_panel_default`), **cookie transparency** (essentials table + Clear-non-essential-local-data button — no real consent toggles because v1 has no analytics/marketing cookies to gate), and **self-service account deletion** (hard delete via `auth.admin.deleteUser` + typed-email confirm + last-admin guard via new `count_org_configure_holders(org_id)` SECURITY DEFINER RPC).

**One migration** (`20260524120000_phase17_settings_redesign.sql`): `orgs.logo_url text` + `profiles.sidebar_hidden_items text[]` + `profiles.argus_panel_default boolean` + new `user_notification_silences (profile_id, notification_kind) PK` table with RLS = `auth.uid() = profile_id` + new permission key `org:configure` granted to `site_admin` only (via idempotent backfill + `seed_default_roles()` patch) + new public Storage bucket `org-logos` with per-org path prefix RLS (`(storage.foldername(name))[1] = p.org_id::text`) + 4 storage policies (read public; insert/update/delete gated on `org:configure`). New helper `canAnywhere(perm)` in `lib/auth/can.ts` for org-scope permission checks (vs the existing site-scoped `can(perm, siteId)`); used by `updateOrg` + `uploadOrgLogo` + `deleteOwnAccount`. **6 new server actions** in `app/(app)/settings/actions.ts`: `updateOrg` · `uploadOrgLogo` · `removeOrgLogo` · `setSidebarHiddenItems` · `setArgusPanelDefault` · `setNotificationSilence` · `deleteOwnAccount`.

**Cross-cutting wires:** `AppSidebar` filters its rendered items against `profiles.sidebar_hidden_items` (after the existing RBAC gate; `HIDEABLE_NAV_ITEMS` const exported from `nav-config.ts`; pinned items `/dashboard` + `/incidents/new/1` + `/admin` are never hideable); `NotificationBell` + `DashboardAlertsBanner` queries add a `WHERE kind NOT IN (silenced)` subquery (defense-in-depth at the read layer so direct DB inserts of life-safety silences still get filtered out); `ArgusContextProvider` reads `argus_panel_default` as initial open state.

**Dropped from earlier draft per Q&A:** Branding tab (folded into Organization as one logo field; brand color stays locked at `#735CDD`); Integrations move (`/admin/integrations` stays put per user direction); Billing + Invoices tabs (no billing model in this app — SDS Manager parent handles licensing); Privacy & Data tab as originally conceived (replaced by Cookies + Delete account). **Tab name for the sidebar-customisation surface: "Sidebar".** **Routes (9 nested under `/settings`):** `/profile` · `/appearance` · `/sidebar` · `/organization` (gated on `org:configure`) · `/argus` · `/notifications` · `/security` · `/cookies` · `/delete-account`; bare `/settings` redirects to `/settings/profile`. **3 SPEC deltas to fold at execution:** (1) §15 entry closing the Phase 8 per-user-notification-silencing deferral; (2) new §AUTH-DELETE-ACCOUNT subsection locking hard-delete semantics + cascade SET NULL behaviour on authored rows + last-admin guard + typed-email confirm + `/login?account_deleted=1` redirect; (3) new §UI-SETTINGS-IA subsection locking the 4-group / 9-tab layout. **8 open Qs** at the bottom of the plan resolve at execution kickoff.
...
Merged 2026-05-12 (PR #45, squash `c975edc`). Slim the `/onboarding` wizard from 3 steps to 2 (workspace basics + use-case multi-select) and add a persistent **Get Started** admin checklist with auto-detected completion on `/get-started`, a compact 3-up `<GetStartedWidget>` on `/dashboard`, and a `<SidebarGetStartedChip>` in the sidebar footer (expanded label-and-bar mode + collapsed 32px circular progress ring). Audience is `org:configure` only (Phase 17 permission). Workers + supervisors keep today's `WorkerWelcomeCard` / `RoleWelcomeCard` — role-tailored worker checklists deferred.

**Two migrations.**

`20260525120000_phase18_onboarding_get_started.sql` — adds `orgs.onboarding_use_cases text[] not null default '{}'` + `orgs.onboarding_dismissed text[] not null default '{}'`, with a one-shot backfill that infers `onboarding_use_cases` for existing orgs from real activity across `incidents` (excluding sandbox + soft-deleted), `inspections`, `hazards`, `jsas`, `assets`, `documents` (excluding archived), and `template_assignments` (via sites join — Planner has no own table, so an active `template_assignments` row stands in for "uses the planner"). Silent orgs land with `'{}'` and see only the always-shown Workspace section. No DB CHECK constraint on the array values — kept open so future tile additions don't need a migration; validation lives server-side in a Zod enum.

`20260525130000_phase18_orgs_update_policy.sql` — **fix recipe**: `init.sql` shipped `orgs` with RLS enabled but only an `orgs_read` SELECT policy. `supabase-js .update()` calls (Phase 17's `updateOrg` and every Phase 18 mutating action — `dismissItem`, `undismissItem`, `updateUseCases`, plus `finishOnboardingWithUseCases`) silently returned `ok` with **0 rows affected** because no UPDATE policy applied. Hidden bug that survived Phase 17 because the wizard test path is rarely exercised end-to-end. New `orgs_update` policy gates writes on `id = current_org() and has_org_permission('org:configure')` — same guard the server actions already enforce in TS via `orgCan('org:configure')`. **Any new column on `orgs` that a user action writes to inherits this policy** — no per-column work needed.

**New `lib/get-started/` slice** (4 files). `use-cases.ts` exports `USE_CASE_KEYS` (5-element `as const` tuple: `incidents` · `inspections` · `hazards_jsa` · `assets_documents` · `planner`), `UseCaseKey` type, `USE_CASES` array with lucide icons + descriptions, and an `isUseCaseKey` guard. `sections.ts` exports `SECTION_KEYS` + `SECTIONS` (workspace + 5 per-use-case + power_up). `items.ts` is the **16-item master catalog** — each entry has stable string id, section, optional `useCase`, label, optional description, `ctaLabel`, `ctaHref` (null = info-only — never appears in the dashboard widget), and a `predicate(ctx) => Promise<boolean>` that runs against Supabase. `state.ts` exports `getChecklistState({ orgId, userId })` — one query reads `orgs.onboarding_use_cases` + `onboarding_dismissed` + `argus_enabled`, applicable items are filtered by use-case + Argus gate, predicates run in parallel via `Promise.all` (each `.catch(() => false)` so one failure can't blank the page), rows grouped into sections (empty sections dropped); plus `pickNextItems(state, limit)` for the dashboard widget. **Counter:** numerator = items where predicate true OR id in `onboarding_dismissed`; denominator = visible items.

**RSC → Client serialization fix recipe.** `ChecklistItem` carries a `predicate` function. When the page passes `ChecklistRowState[]` to client components (`<ChecklistRow>`, `<ShowDismissedToggle>`, `<GetStartedWidget>`), React throws *"Functions cannot be passed directly to Client Components"*. Fix lives in `state.ts` — `SerializableChecklistItem = Omit<ChecklistItem, 'predicate'>` and assembly strips the field: `const { predicate: _p, ...itemSafe } = item; void _p;`. **Pattern for future RSC → Client row data:** when an RSC computes rich rows with non-serializable fields, define a `Serializable*` alias and strip those fields at assembly time.

**Onboarding wizard rewritten in place** (`app/(onboarding)/onboarding/`). `onboarding-wizard.tsx` collapses to 2 steps — `OrgAndSiteStep` unchanged + new `UseCasesStep` that renders the shared `<UseCaseTiles>` and submits multi-value hidden inputs (`<input name="use_cases" value={k}>` per selected tile). "I'm not sure yet — show me everything" link selects all 5. `actions.ts` drops `inviteOnOnboarding` + the old `finishOnboarding`; new `finishOnboardingWithUseCases` writes `orgs.onboarding_use_cases`, stamps `profiles.onboarded_at`, and redirects to `/dashboard?welcome=1`. `page.tsx` simplified — no `searchParams` prop; resume logic reads `site_members` presence to detect "step 1 already done" and forces step 2. **Drive-by fix:** the wizard progress bar used `bg-brand` (silently broken per existing memory `feedback_bg_brand_not_a_utility`) — flipped to `bg-primary`.

**Route `/get-started`** (`app/(app)/get-started/`) — sectioned RSC, gated on `orgCan('org:configure')` (redirects non-admins to `/dashboard`). Header carries title + progress bar + `<EditUseCasesModal>` trigger ("Change what you use"); body renders one section per surviving use-case; `<ShowDismissedToggle>` at the bottom reveals dismissed rows with un-dismiss affordance. At 100% (`doneCount >= totalCount && totalCount > 0`) the body flips to a centered celebration card with a "Go to dashboard" CTA. `actions.ts` exports `dismissItem(id)` · `undismissItem(id)` · `updateUseCases(keys[])`, all gated on `requireOrgPermission('org:configure')`, each revalidating `/get-started` + `/dashboard` + the layout. **Action return shape:** `{ ok: true }` (omitting `data`) — `ActionResult<T = void>` types `data` as `void`, so `{ ok: true, data: null }` is a type error.

**Components `components/get-started/`** (6 files). `use-case-tiles.tsx` — shared tile renderer used by both the wizard step 2 and `<EditUseCasesModal>`. `checklist-row.tsx` — list-item primitive with done/dismissed/pending states (`useTransition` for dismiss/undismiss server-action calls). `show-dismissed-toggle.tsx` — collapsible "Show N dismissed items" toggle; returns null when empty. `dashboard-widget.tsx` — compact RSC with progress bar + up to 3 next not-done-not-dismissed CTA rows + "View all →" to `/get-started`. `sidebar-get-started-chip.tsx` — client component (uses sidebar collapse state); SVG circular progress ring + Sparkles icon in collapsed mode (32px), label + counter + bar in expanded mode. Tooltip shows in both states (simpler than conditional Tailwind data-selector).

**Mounts.** `app/(app)/dashboard/page.tsx` extends the existing `Promise.all` to fetch `orgCan('org:configure')` + `getChecklistState` alongside the Argus tile payloads, then renders the widget between the existing yellow `setupIncompleteSiteName` banner and the "Hi, {firstName}" heading **only when `isOrgAdmin && doneCount < totalCount`**. The yellow banner is **kept** — both nudges coexist (banner is urgency-flavored, widget is checklist-flavored). `app/(app)/layout.tsx` folds `orgCan('org:configure')` into its existing `Promise.all`, computes `checklist` sequentially when `isOrgAdmin`, and passes `getStartedCounts={ done, total } | null` down through `<SidebarShell>` to `<AppSidebar>`. `AppSidebar` renders `<SidebarGetStartedChip>` above the user-info block in `<SidebarFooter>` — the `group-data-[collapsible=icon]:hidden` class moved off the footer onto the inner user-info `<div>` so the chip's collapsed-mode ring stays visible.

**SPEC delta** — new 2026-05-12 row in §15 Decisions log + new **§UI-GETSTARTED** subsection (between §UI-SETTINGS-IA and §AUTH-DELETE-ACCOUNT) locking the 5 use-case keys, the 16-item master catalog with auto-detect predicate descriptions per row, the counter semantics, the 100% behavior (widget + chip hide; page renders celebration), and the `org:configure`-only audience.

**Worth noting** — `first_investigation_run` predicate uses `.eq("status", "closed")` instead of `.not("root_cause_finalized_at", "is", null)`. The plan originally specified the latter column but it doesn't exist on `investigations`; `closed` is the terminal state set when all investigation work completes, which is the correct signal.

**Kickoff Q&A resolutions** (locked during brainstorm 2026-05-12): wizard scope = slim (org + site basics + module pick — invite step dropped to Get Started item) · module picker = use-case tiles, not module names (5 friendly use-cases) · completion = hybrid auto-detect + per-org dismiss (no materialized rows) · audience = admin-only (workers stay on welcome cards) · existing orgs backfill use-cases from real activity · 100% auto-hides widget + chip, page stays alive · widget shape = compact 3-up preview (Linear/Vercel/Intercom default) · sidebar chip above user-info footer (not merged) · /get-started IA = sectioned (Workspace + per-use-case + Power up) · yellow banner stays · un-dismiss via toggle · "Change what you use" modal.

**Deferred to v2 / Phase 18.5** (hard cuts): worker / supervisor tailored checklists · per-user dismissal (vs per-org) · animation/confetti at 100% · sample-data seeding (no dummy incidents/templates/hazards on signup) · Argus item beyond the existing `argus_intro_seen` row · email-driven re-engagement nudges.

`pnpm tsc --noEmit` clean. `pnpm lint` matches `main` baseline (zero new warnings). Plan: `plans/18-onboarding-get-started.md`. Spec: `docs/superpowers/specs/2026-05-12-onboarding-get-started-redesign-design.md`.

---

## Phase 19 — Org-level Dashboard + per-site Dashboard

Six small PRs (19a-19f); first two shipped, rest queued. Flips `/dashboard` from site-scoped to org-scoped, adds a new public per-site dashboard at `/sites/[id]`, lays in the Leaflet sites map, and lines up the per-section sub-components so 19c/19d/19e/19f each ship cleanly. Plan: `plans/19a-…f-*.md`. Spec: `docs/superpowers/specs/2026-05-12-org-dashboard-design.md`.

### Phase 19 design + 6 implementation plans

Merged 2026-05-12 (PR #46, squash `d1a00ac`). Docs-only PR: spec at `docs/superpowers/specs/2026-05-12-org-dashboard-design.md` + 6 plan files (`plans/19a-org-dashboard-foundation.md` through `plans/19f-per-site-dashboard.md`). Locks: per-site dashboard at `/sites/[id]` (new public route, separate from `/admin/sites/[id]`); stacked single-column layout matching `assets/dashboard.png`; 3x2 module cards grid (Incidents · Inspections · Hazards · JSA · CAPA · Investigations); Leaflet + OpenStreetMap tiles (no API key); "My drafts" semantics (resume-where-I-left-off, not org admin oversight); New bulletin added to the Quick actions row for `bulletin:create` holders; six-plan decomposition with 19a as the only hard prerequisite. Plan: docs-only, no migrations.

### Phase 19a — Org Dashboard Foundation

Merged 2026-05-12 (PR #47, squash `c1284e7`). Topbar `<SiteSwitcherSlot>` Client wrapper reads `usePathname()` and renders null on `/dashboard` only — mobile in-sheet `<SiteSwitcher>` in `<AppSidebar>` is intentionally unaffected (renders on every route including `/dashboard`, by design — the desktop hide is the visible signal of org-mode but mobile sheet UX stays consistent). `/dashboard` KPIs flipped from `currentSiteId`-scoped to `org_id`-scoped; 5th KPI **Active sites** (`count(*) from sites where org_id = current_org() and setup_completed_at IS NOT NULL`) prepended with brand-purple accent via `border-primary/30 bg-primary/5 text-primary`; grid widened from `md:grid-cols-4` to `lg:grid-cols-5`. TRIR/DART hours-worked now sums across the org via `site_annual_hours.select("hours_worked, site:sites!inner(org_id)").eq("site.org_id", orgId)` (PostgREST dotted filter through embedded relation alias — first place in the codebase to use the pattern; existing `user_can_access_site` RLS on `site_annual_hours` provides per-tenant scoping).

**Argus tile aggregators support `siteId: null` = org-wide.** All 4 tile files in `lib/argus/tiles/` had their early `if (!siteId) return null;` guard removed and rewritten as `let q = supabase.from(...).select(...)...; if (siteId) q = q.eq("site_id", siteId); const { data, error } = await q;`. The post-query reduce/aggregation logic is unchanged. **Companion change:** the tile POST handler at `app/api/argus/tile/route.ts` widened its Zod schema for `siteId` from `z.string().uuid()` to `z.string().uuid().nullable()` and branches: `payload.siteId ? await can(config.permission, payload.siteId) : await orgCan(config.permission)`. `components/argus/argus-insight-tile.tsx` + `components/argus/use-argus-insight-tile.ts` widened the `siteId` prop type from `string` to `string | null`. **Important: this companion route+component change was NOT in the original plan** — without it, the "Analyse" button would 400-reject on `/dashboard` while the render path looked fine. Worth flagging as a pattern: when an aggregator's signature widens to nullable, audit every consumer (tile renderer, POST handler, log function) for matching widening.

`/dashboard` permission gates switched from `currentSiteId ? can(perm, currentSiteId) : Promise.resolve(false)` to `orgCan(perm)` for the 4 Argus tiles + introduced `canStopWorkRead` via `orgCan("incident:read_site")` replacing the dead `canReadSite` variable. Dashboard subtitle copy updated from "Site-scoped activity, regulatory clocks, and quick actions." to "A live picture of incidents, inspections, and risk across your sites." `let` → `const` for the 6 KPI vars that were inherited from the old `if (canReadSite && currentSiteId)` gate.

**Two blocks deleted from `/dashboard`:** the Recent Incidents `<section>` (returns reshaped in 19e as org-wide activity feed) and the "Reporting starts here" worker empty state (superseded by 19c's Quick actions row). 7 imports cleaned up (`redirect`, `ArrowRight`, `INCIDENT_TYPE_META`, `IncidentType`, `SeverityBadge`, `TrackBadge`, `StatusBadge`).

**New route `/sites/[id]`** — stub placeholder so 19b's map pins have a navigation target. Permission-gated: `notFound()` on missing site OR cross-org access (prevents resource-existence leakage); `redirect("/dashboard")` on missing `incident:read_site` perm (preserves confidentiality once existence is known). Stub body: site name h1 + amber "Site setup not yet finished" banner when `setup_completed_at IS NULL` + Construction-icon placeholder card + 3 deep-link CTAs (View incidents · View inspections · Manage site). `params: Promise<{ id: string }>` per Next 16. Real implementation lands in 19f.

**Two stale tile docstrings refreshed** in `lib/argus/tiles/{overdue-investigations,capa-overdue,stop-work-active,reportability-uncertain}.ts` — wording acknowledges org-wide fan-out when `siteId === null`, scopes to site when set, returns null only on DB error. The plan called for the first two; the holistic pre-PR review caught two more.

CLAUDE.md hard rules added:
- **`/dashboard` is org-scoped; `/sites/[id]` is the per-site dashboard; topbar SiteSwitcher is hidden on `/dashboard` only.** Permission gates: `/dashboard` uses `orgCan(perm)`, `/sites/[id]` uses `can(perm, siteId)`.

`pnpm lint` matches main baseline (87 problems, −1 warning from cleaned imports vs main's 88). `pnpm build` clean — `/sites/[id]` registered as a PPR route. Plan: `plans/19a-org-dashboard-foundation.md`.

### Phase 19b — Sites Map on Org Dashboard

Merged 2026-05-12 (PR #48, squash `1d07636`). Full-width Leaflet sites map on `/dashboard` between the KPI strip and the Argus tiles block. Pin color reflects site posture: green (`#16a34a`, clean), amber (`#d97706`, open S1/S2), red (`#dc2626`, active stop-work). Click navigates to `/sites/[id]`. Empty state when zero sites in the org have coordinates; footnote when partial coverage ("N site(s) hidden — missing coordinates").

**New deps:** `leaflet@1.9.4` + `react-leaflet@5.0.0` + `@types/leaflet@1.9.21` (dev). **v5 installed instead of plan's v4** — v5 declares `"react": "^19.0.0"` as a peer dep, no warnings. Surface area used (MapContainer + TileLayer + CircleMarker + Tooltip + eventHandlers) is unchanged between v4/v5. `@types/leaflet`'s `LeafletKeyboardEvent` typed import used in the keydown handler instead of the `@ts-expect-error` cast the plan suggested. `public/leaflet/marker-{icon,icon-2x,shadow}.png` staged for future `<Marker>` usage (today's map uses `CircleMarker` — no default icon needed at runtime). `app/globals.css` imports `leaflet/dist/leaflet.css` as the first import, before Tailwind so utility classes can still override.

**Three-file SSR boundary** (now a CLAUDE.md hard rule). Next 16 with `cacheComponents: true` rejects `next/dynamic({ ssr: false })` from Server Components. Architecture:
- `components/dashboard/sites-map-card.tsx` (**Server**) — runs `getSitesMapPayload(orgId)`, renders empty state inline when `markers.length === 0`, otherwise renders the section header + footnote and delegates the live render to the Client bridge.
- `components/dashboard/sites-map-client.tsx` (**Client bridge**, `"use client"`) — the only file that calls `dynamic(() => import("./sites-map").then((m) => m.SitesMap), { ssr: false, loading: skeleton })`. 18 LOC, exists solely to host the dynamic import.
- `components/dashboard/sites-map.tsx` (**Client Leaflet**, `"use client"`) — `<MapContainer>` + `<TileLayer>` + colored `<CircleMarker>` per marker + `<Tooltip>` with site name + posture + "Click to open →".

**Aggregator at `lib/dashboard/org/sites-map.ts`** — 3 queries total (1 sites + 2 parallel incident rollups), all org-scoped: sites with NULL lat/lng counted as `hiddenForMissingCoords`; S1/S2 open count per site; active stop-work flag per site. **Schema correction discovered during build:** the plan's `.is("stop_work_lifted_at", null)` referenced a non-existent column. The implementer correctly substituted `.is("stop_work_acknowledged_at", null)` to match the canonical "active stop-work" definition in `lib/argus/tiles/stop-work-active.ts:24` (the existing convention is "active = stop_work=true AND not yet acknowledged by management"; there is no separate "lifted" lifecycle in the schema).

**Known limitation noted in PR description:** keyboard navigation on map pins is inert. Leaflet renders `<CircleMarker>` as an SVG `<path>` element which is not focusable (no `tabIndex`). The `eventHandlers.keydown` handler in the Client component is present but never fires. Sites remain keyboard-reachable via SiteSwitcher and via 19c's upcoming Quick actions / module cards. Switching to `<Marker>` would fix the focus issue but lose the colored-circle visual encoding; deferred. Acceptable for v1.

**`bounds` + `center` + `zoom` are all passed simultaneously when `markers.length >= 2`** — Leaflet runtime uses `bounds` (`fitBounds`) and ignores center/zoom when bounds is set. TypeScript accepts because `MapOptions` types them as independent optionals. Single-marker case uses `center+zoom={10}`; multi-marker uses `bounds`. Logic is in `sites-map.tsx`.

Mounted on `/dashboard` between the 5-up KPI strip and the `{argusAvailable && (...)}` Argus tiles block: `<SitesMapCard orgId={profile.org_id} />`. The aggregator's `createClient()` call opts the section into dynamic rendering — consistent with the existing `LatestBulletinsCard` pattern on the same page.

CLAUDE.md hard rule added:
- **Browser-only Client libraries need a three-file SSR split under Cache Components.** Server card → Client bridge with `dynamic({ ssr: false })` → real Client Component importing the browser-only lib. Don't short-circuit `dynamic({ ssr: false })` from a Server Component — build fails.

`pnpm lint` matches main (87 problems, identical to base). `pnpm build` clean — sites map adds ~50 KB gzipped to the `/dashboard` chunk as expected. Plan: `plans/19b-sites-map.md`.

### Phase 19c — Quick Actions Row + Module Cards Grid

Merged 2026-05-12 (PR #50, squash `555ac20`). Quick actions row directly under the `/dashboard` greeting; 3x2 module cards grid below the Argus tiles block, above `LatestBulletinsCard`. Six quick-action CTAs (Report incident · Start inspection · Add hazard · New CAPA · New JSA · New bulletin), each gated on its create permission and the primary "Report incident" rendered in brand-purple. Six module cards (Incidents · Inspections · Hazards · JSA · CAPA · Investigations), each with two numbers + an arrow, gated by its read permission and dropping when the user lacks access.

**Six aggregators at `lib/dashboard/org/modules/{incidents,inspections,hazards,jsa,capa,investigations}.ts`** — all share the signature `(supabase, orgId, siteId) => Promise<ModuleCardData>` returning `{ primary: { label, value }, secondary: { label, value, tone: "neutral" | "warn" | "alert" } }`. When `siteId` is `null`, filter by `org_id`; when set, filter by `site_id`. Same pattern Phase 19a established for Argus tiles; reused now and forwards-compatible with 19f's per-site dashboard.

**`<ChooseSiteSheet>` picker pivots from the plan's `?site=<id>` URL contract to the existing `setSelectedSite(siteId)` server action (Phase 12).** On pick, the sheet sets the topbar site cookie and navigates — no per-wizard URL contract needed, and the topbar stays in sync after the wizard. New CLAUDE.md hard rule. Zero wizard patching required (Report Wizard, `/hazards/new`, `/jsa/new`, `/capa?action=create` modal, `/inspections` Start dialog all already read `currentSiteId` from `requireUser()`). `/bulletins/new` is org-level so the picker is skipped entirely for "New bulletin".

**Plan deltas absorbed at edit time (caught by grep-before-commit, per `[[feedback-verify-columns-exist]]`):**
- **Permission keys:** plan referenced `hazard:read` / `hazard:create` / `jsa:read` / `jsa:create` (don't exist); aligned to live `hazard:read_site` / `hazard:report` / `jsa:read_site` / `jsa:draft`. Module-card read gates use `*:read_site` flavours plus `capa:complete` and `investigation:lead` (no dedicated read perm exists for those — same gating the list pages already use).
- **Hazards `residual_severity` doesn't exist on `hazards`** — `residual_risk_score` lives on `hazard_risk_assessments` joined via `current_risk_assessment_id`. The aggregator mirrors the `app/(app)/hazards/page.tsx` pattern: `select("hazard_id, hazards!inner(id, deleted_at, status, site_id, org_id)", { count: "exact", head: true })` with the org/site filter on the embedded relation.
- **JSAs `retired_at` + `next_review_at` don't exist** — soft-delete uses `deleted_at`; expiry uses `expires_at` (date, set on approval to `approved_at + 12 months` by default).
- **`capa_status` enum has no `abandoned`** — Open = `not in (verified,closed)`. The plan's `("verified","closed","abandoned")` filter trimmed to two values.
- **`inspections` has no scheduling column at all** — schedules live on `template_assignments.schedule_cron` (cron strings, not fixed dates). The plan's "Due this week / Overdue" metric pair isn't computable from the schema. Pivoted to **In progress + Failed (14d)** mirroring `lib/argus/tiles/inspections-due-summary.ts`. User-confirmed at kickoff.
- **Quick-action destinations:** `/inspections/new` and `/capa/new` don't exist. Replaced with `/inspections` (the list page hosts `StartInspectionDialog`) and `/capa?action=create` (the list page hosts the standalone-create modal).

`/dashboard` modifications: six new `orgCan()` flags (`incident:report` · `inspection:run` · `hazard:report` · `capa:create` · `jsa:draft` · `bulletin:create`) folded into the existing parallel `Promise.all`; the sites query (`sitesRes`) flipped from `count: "exact", head: true` to fetching `id, name` rows ordered by name (the picker reads these; `activeSitesCount` reads `.length` from the same result, so still one round-trip). Standalone Report-incident button block dropped — it's the primary CTA inside `<QuickActionsRow>` now. Greeting div + buttons restructured from `flex justify-between` to greeting-on-top + row-below.

CLAUDE.md hard rule added:
- **Cookie-based site switching is the canonical org-mode → wizard pivot, not `?site=` URL params.** Use `setSelectedSite(siteId)` from `components/app-shell/site-actions.ts` then `router.push(destination)`. Future quick-action surfaces, dashboard CTAs, and any org-scope page that needs to drop the user into a site-scoped wizard should reuse it — keeps wizards unchanged and keeps the topbar in sync.
...
Merged 2026-05-12 (PR #52, squash `ba62145`). Two recharts panels on `/dashboard` between the module cards grid and `<LatestBulletinsCard>`. Left: `<AreaChart>` of incidents per month over the last 12 months, stacked by severity (S1 `#dc2626` · S2 `#d97706` · S3 `#eab308` · S4 `#16a34a` — the project's success green for the lowest tier). Right: `<PieChart>` donut (inner 50 / outer 80) of severity distribution YTD. Both render a muted empty-state card when the underlying data sums to zero — recharts has no good native empty state.

**Two aggregators at `lib/dashboard/org/{incidents-trend,severity-distribution}.ts`** — same `(supabase, orgId, siteId) => Promise<...>` signature 19a/19c established so 19f reuses them. The trend aggregator pre-seeds 12 month buckets so empty months render as zero rather than dropping out of the series. One round-trip each; the section wrapper runs them in parallel via `Promise.all`. Org/site filter chosen based on whether `siteId` is null.

**Two plan deltas absorbed at edit time:**
- **CSS tokens use Tailwind v4 raw vars, NOT the v3 shadcn `hsl(var(--*))` wrap.** Plan referenced `hsl(var(--border))` and `hsl(var(--muted-foreground))`. Project's `@theme` block defines `--color-border: var(--border)` and `--color-muted-foreground: var(--foreground-muted)`, so the raw token names recharts inline styles need are `var(--border)` and `var(--foreground-muted)`. Existing precedent in `components/incidents/list/incidents-over-time.tsx`. **General rule for any future chart additions: grep an existing chart file before trusting shadcn docs verbatim.**
- **Recharts 3.x `<Tooltip formatter>` signature**: `Formatter<ValueType, NameType>` now declares both as `… | undefined`. Dropped the explicit `(value: number, name: string)` annotations the plan suggested; let inference do its job and coerce the returned name via `String(name)`.

**No SSR bridge needed for recharts.** Charts are plain `"use client"` files. The three-file split rule from 19b only applies to libs that touch `window` at module load. `pnpm build` confirmed.

`pnpm lint` matches main baseline. `pnpm build` clean. Plan: `plans/19d-trends-charts.md`.

### Post-19d Hotfix — Leaflet CSS import + dead `use server` type import

Merged 2026-05-12 (PR #53, squash `2e1ccf8`). Two unrelated regressions surfaced after the 19d merge — both caused runtime / build failures on `main`.

**1. Leaflet CSS import location.** The 19b setup `import "leaflet/dist/leaflet.css"` from `app/globals.css` failed under Tailwind v4's PostCSS pipeline (couldn't resolve `node_modules` from a CSS `@import`). Moved the import into the leaf Client Component `components/dashboard/sites-map.tsx` so it loads via the JS bundler (which resolves `node_modules` cleanly) only when the map mounts. **Updated the three-file SSR split hard rule in CLAUDE.md** to include: when a browser-only library ships its own CSS, import it from the same `"use client"` file that imports the library — NOT from `app/globals.css`.

**2. Dead `import type { MatrixCoord }` in `app/(app)/incidents/[id]/actions.ts`.** Latent since Phase 1 (`fbf41d0`) — never used in the file body, but Next's `"use server"` registration shim was emitting a runtime reference to the type identifier. `overrideSeverity` / `assignTriageOwner` were throwing `ReferenceError: MatrixCoord is not defined` when invoked. Deleted the unused import. **This sharpens the existing `[[feedback-use-server-exports]]` rule**: the prohibition on non-async exports extends to type-only IMPORTS that aren't used in the file — SWC emits a runtime stub if it can't statically prove the type is erasable in this position. Grep `"use server"` files for unused `import type` and delete them.

### UI Polish — Gradient + counter animations on dashboard and incident wizard

Merged 2026-05-13 (PR #62, squash `3124ea4`). Cross-surface visual polish on `/dashboard`, `/incidents` (hero KPI strip), and the `/incidents/new/{1,2,3}` wizard. No schema changes, no new perms, no new RPCs, no data flow changes — every value still originates server-side and the new primitives hydrate on the client.

**New `<AnimatedNumber>` primitive at `components/ui/animated-number.tsx`.** Client component, accepts `string | number`. Eases from `0 → target` on mount via `requestAnimationFrame` with cubic-ease-out (`1 - (1-t)^3`), preserves decimal precision from the input (`"35.0"` animates with one decimal place, `"12"` rounds to integer), passes non-numeric strings (`"—"`, `"n/a"`) through verbatim. Re-arms when the `value` prop changes so post-mutation data also animates. Reads `prefers-reduced-motion` inside the effect and snaps straight to the final value when set. Wired into:
- Dashboard `<AnimatedKpiStrip>` (5 KPIs incl. emphasis card)
- Dashboard `<ModuleCard>` primary + secondary numbers (six tiles, secondary delayed 120ms so the cascade is legible)
- `/incidents` hero `<KpiCard>` (text-3xl xl:text-4xl numbers)

**`<AnimatedKpiStrip>` at `components/dashboard/animated-kpi-strip.tsx` (new).** Wraps the 5-up KPI grid; emphasis card ("Active sites") gets a `from-primary/10 via-primary/5 to-transparent` gradient backdrop, a soft corner glow blob, and a pulsing dot + ring (uses `animate-pulse-soft` + `animate-pulse-ring` utilities from `globals.css`). Per-card stagger via the inline `--d` custom property consumed by the `dashboard-enter` utility.

**`app/globals.css` additions (4 keyframes + 4 utilities + reduced-motion guard).**
- Keyframes: `pulse-soft`, `pulse-ring`, `aurora-drift`, `shimmer-sweep`.
- Utilities: `aurora-blob` (`position: absolute; filter: blur(60px); opacity: 0.55; animation: aurora-drift 18s ease-in-out infinite`), `animate-pulse-soft`, `animate-pulse-ring`, `dashboard-enter` (`animation: enter 600ms cubic-bezier(0.22, 1, 0.36, 1) var(--d, 0ms) both` — uses tw-animate-css's existing `@keyframes enter` and reads `--d` for per-element delay).
- Single `@media (prefers-reduced-motion: reduce)` block disables all four with `animation: none !important` and dims aurora blobs to 0.35.
- **`dashboard-enter` is the canonical stagger utility going forward** — used on the dashboard sections, the incident-wizard sections, and the Step 1 type-picker tiles. Reuse for any new fade-up entry sequence.

**Three-file SSR split is NOT required for these animations.** The new primitives touch `window` only inside `useEffect` (`window.matchMedia?.("(prefers-reduced-motion: reduce)").matches`), which runs post-hydration. No `dynamic({ ssr: false })` needed.

**Dashboard `/dashboard`.** Wrapper switched from `space-y-6` to `relative isolate space-y-6`. Three drifting aurora blobs in an `inset-x-0 -top-12 -z-10 h-[520px] overflow-hidden` div (brand purple top-left, accent-cyan right, brand-hover center; opacities 0.32 / 0.28 / 0.35 with staggered `animationDelay` so the drift desyncs). Greeting block converted from a plain heading into a `from-primary/10 via-card to-card` gradient hero card with corner glow and `bg-clip-text` gradient on the "Hi, Rafi" headline. Section-by-section `dashboard-enter` (60ms cadence: Argus 60 → greeting 60 → quick-actions 120 → KPI strip ~120-360 → sites map 420 → Argus tiles 480 → modules 540 → trends 600 → activity 660 → bottom 720).

**Module cards (`components/dashboard/module-card.tsx`).** Hover gets `-translate-y-0.5` + `hover:shadow-md` + `hover:border-primary/40`; gradient top accent rail (`from-primary via-accent-cyan to-primary`) fades in on hover; soft brand glow blob (top-right corner, `bg-primary/10 blur-2xl`) appears on hover; icon swatch border/bg/text shift to brand purple on hover; arrow translates `translate-x-1` further than the previous `translate-x-0.5`. All transitions `duration-300`. `motion-reduce:hover:translate-y-0` opt-out.

**Quick actions row primary CTA (`components/dashboard/quick-actions-row.tsx`).** Brand-purple gradient (`from-primary via-primary to-[var(--brand-hover)]`) with a left-to-right shine sweep — a single `bg-gradient-to-r from-transparent via-white/25 to-transparent` overlay that translates from `-translate-x-full` to `translate-x-full` over 700ms on hover. Wrapped in `overflow-hidden`; the icon and label sit on `relative` to render above the sweep. `motion-reduce:hidden` on the sweep, `motion-reduce:hover:translate-y-0` on the button.

**Same shine-sweep pattern reused on three wizard submit CTAs** (Step 1 "Continue to Step 2", Step 2 "Continue to Step 3", Step 3 "Finalize report"). Identical structure: `group relative inline-flex … overflow-hidden bg-gradient-to-br from-primary via-primary to-[var(--brand-hover)] … transition-all duration-300 hover:-translate-y-0.5` + the `pointer-events-none absolute inset-0 -translate-x-full` overlay span. **This is the canonical "primary CTA" treatment going forward.**

**Trends section (`components/dashboard/trends-section.tsx`).** Each of the two chart cards gains a thin (`h-0.5`) gradient accent rail at the top (left card `from-primary/70 via-accent-cyan/60 to-transparent`, right card mirrored `from-transparent via-primary/60 to-primary/70`). Cards now `relative overflow-hidden` with `hover:shadow-sm`.

**Incident wizard page (`app/(app)/incidents/new/[step]/page.tsx`).** Same aurora-backdrop pattern as the dashboard, three blobs (brand + accent-cyan + brand-hover), `inset-x-0 -top-10 -z-10 h-[420px]`. Header card: `border-primary/15` + `bg-gradient-to-br from-primary/10 via-card to-card` + `from-primary via-accent-cyan to-primary/40` top rail + brand-tinted icon swatch (`from-primary/20 to-primary/5 text-primary ring-1 ring-primary/30`) + gradient-clipped title (`from-primary via-foreground to-foreground bg-clip-text text-transparent`). **The header was shipped originally with destructive/warning colors; the reviewer rejected them as off-brand mid-PR and they were swapped to brand-purple in `3e1ecb4`.** Future incident-related surfaces should follow the same rule: semantic destructive tone stays on real "incident is serious" affordances (severity badges, stop-work banner, destructive-action confirms); ambient surfaces (page headers, hero cards) should stay on brand.

**Rebuilt `<WizardProgress>` (`components/incidents/wizard/wizard-progress.tsx`).** Active step pill: `from-success/15 to-success/5` gradient + outer blur halo (`from-success/40 via-success/20 to-success/40 opacity-60 blur-md`) + filled number badge with success gradient + `animate-pulse-ring`. Completed steps: success/85 fill. Connector between steps animates from `bg-border` to `bg-gradient-to-r from-success/70 to-success/40` as you cross it. **New: slim 1px gradient progress rail** under the pills with `transition-[width] duration-700 ease-out` and width set to `((current - 1) / 2) * 100%` — purely decorative reinforcement of progress. `SandboxBanner` (still exported from this file even though it's no longer mounted by the wizard) gains a gradient wash + amber vertical accent stripe.

**Step 1 type-picker tiles (`components/incidents/wizard/step-1-what-happened.tsx`).** 8-up grid where each button gets `dashboard-enter` with `--d: ${240 + i * 50}ms` (so the cascade trails the rest of the page entry). Hover: `-translate-y-0.5 hover:shadow-md` + an `origin-left scale-x-0 group-hover:scale-x-100` gradient rail across the top (the rail uses `transition-transform` rather than the dashboard module-card's opacity fade — both look right; treat them as interchangeable). Selected: `border-primary/60 bg-gradient-to-br from-primary/15 via-primary/5 to-card ring-1 ring-primary/40` + corner glow blob + gradient icon swatch (`from-primary/20 to-primary/5 ring-1 ring-primary/30`) + label flips to `text-primary`.

**Sandbox UI removed from the create wizard (`92238ba`).** `<SandboxBanner />` no longer mounted in any step; the "Practice mode (sandbox)" checkbox + label block removed from Step 1. **The `?sandbox=true` URL-param path is preserved**: Step 1 emits a hidden `<input type="hidden" name="is_sandbox" value={sandbox ? "true" : ""} />` seeded from `initial.is_sandbox || initialSandbox`, so demo flows (`/incidents/new/1?sandbox=true`) still mark the resulting draft as sandbox without surfacing it to ordinary users. `actions.ts` parses `fd.get("is_sandbox") === "on" || fd.get("is_sandbox") === "true"` — both shapes accepted, so nothing else needed changing. `isSandbox` prop on `<Step2Details>` is now received but unused (kept in the prop type so the wizard page's call site stays unchanged); Step 3 still uses `!isSandbox` to gate the regulatory-clock helper text — that conditional stays correct for the URL-param path.

**Pulse ring on the wizard header icon was shipped, then removed mid-review (`7fa3baf`).** The icon retains its gradient swatch + static ring; the pulsing ring read as too urgent for a wizard header. Reserve `animate-pulse-ring` for emphasis on live data ("Active sites" KPI, in-progress step badge in `<WizardProgress>`) — NOT static page-header decoration.

CLAUDE.md hard rules / locked decisions added by this PR — none codified yet (treat the bullets above as the durable conventions). The "primary CTA = brand-purple gradient + shine sweep" pattern, the "`dashboard-enter` stagger via `--d`" pattern, and the "header chrome stays brand even when the surface is incident-themed" rule are likely candidates if a future PR drifts from any of them.

`pnpm lint` matches main baseline (the two pre-existing `<a href="/incidents">` errors in Step 1 + Step 3 from Phase 1 / Phase 9b remain — not introduced by this PR). `pnpm build` was not run on the merge commit; the dev server (Turbopack) recompiled all touched routes cleanly across multiple iterations.

---

## Workflow notes

All v1-roadmap phases (0-13) shipped + Phase 7 Global Search + Phase 10 Safety Bulletins. Phase 9 closed (9a foundation, 9b Copilot, 9c Investigator, 9d Magic Wands + Gemini pivot, 9e Global Panel + Insight Tiles). V2 surface kickoff: Phase 14 Hazard Register + Phase 16 SDS Manager stub shipped together (PR #40); Phase 15 JSA shipped (PR #41); Phase 17 Settings Redesign plan merged (PR #42, code not started). Phase 17 Settings Redesign code shipped 2026-05-12 (PR #44). Phase 18 Onboarding + Get Started shipped 2026-05-12 (PR #45). Phase 19 Org Dashboard kicked off 2026-05-12: spec + 6 plans landed PR #46; Phase 19a Foundation PR #47; Phase 19b Sites Map PR #48; Phase 19c Quick Actions + Module Cards PR #50; Phase 19d Trends Section PR #52 + post-19d hotfix PR #53 (leaflet CSS location + dead use-server type import) — Phases 19e (Activity + drafts/assets/documents) / 19f (per-site dashboard at /sites/[id]) queued and shippable in any order (only 19a was a hard prerequisite). Remaining v2 surface per `PLANNING/IMS_PLANNING.md`: Document Control / Training / Audit / MOC / Permit / Inspection ITA + HSE submission / Risk Register / Contractor / Toolbox Talks / BBS / Emergency Management / Environmental — each needs a fresh scoping pass before being picked up. Every change that affects runtime behavior goes through a feature branch + PR per `.claude/rules/github-workflow.md`. Direct push to `main` is reserved for doc-only updates the user explicitly asks for.
